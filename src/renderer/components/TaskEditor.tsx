...
export default function TaskEditor({ date, tasks, onToast }: TaskEditorProps) {
  ...
  const handleCreateBelow = useCallback(
    (taskId: string, text = '') => {
      ...
      if (parentId) {
        const newId = insertSubtaskAfter(date, parentId, taskId, text)
        requestAnimationFrame(() => setActiveTaskId(newId))
      } else {
        const newId = insertTaskAfter(date, taskId, text)
        requestAnimationFrame(() => setActiveTaskId(newId))
      }
    },
          ...
          <>
            <AnimatePresence>
              {tasks.map((task, i) => (
                <EditableTaskRow
                  ...
                  onCreateBelow={(text?: string) => handleCreateBelow(task.id, text)}
                  ...
                  onSubtaskCreateBelow={(id, text) => handleCreateBelow(id, text)}
            ...
            {resolvedRefs.map(({ ref, task: linkedTask }) => (
              <EditableTaskRow
                ...
                onCreateBelow={(text?: string) => {
                  // Enter on linked parent → create subtask
                  const newId = addManualSubtask(ref.originDate, linkedTask.id, text)
                  requestAnimationFrame(() => setActiveTaskId(newId))
                }}
                ...
                onArrowDown={() => {}}
                onSubtaskFocus={(id) => setActiveTaskId(id)}
                onSubtaskCreateBelow={(subtaskId, text) => {
                  // Enter on subtask → create sibling subtask
                  const newId = insertSubtaskAfter(ref.originDate, linkedTask.id, subtaskId, text ?? '')
                  requestAnimationFrame(() => setActiveTaskId(newId))
                }}
