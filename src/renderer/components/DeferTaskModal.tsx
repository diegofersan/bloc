...
export default function DeferTaskModal({ isOpen, onClose, taskId, originDate }: DeferTaskModalProps) {
  const createTaskRef = useTaskStore((s) => s.createTaskRef)

  const today = useMemo(() => new Date(), [])
  const originParsed = useMemo(() => parseISO(originDate), [originDate])
  ...
  function handleSelectDate(date: Date) {
    ...
    createTaskRef(originDate, taskId, dateKey)
    onClose()
  }
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            <h3 className="text-sm font-semibold text-text-primary mb-4">Adiar tarefa</h3>
                ...
                <div key={monthStart.toISOString()}>
                  ...
                  <div className="grid grid-cols-7 gap-0.5">
                    ...
                    {days.map((day) => {
                      ...
                      const isOrigin = isSameDay(day, originParsed)
                      const isPast = day < today && !isToday
                      const disabled = !inMonth || isOrigin || isPast
